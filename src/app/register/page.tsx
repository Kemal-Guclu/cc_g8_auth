...
export default function RegisterPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result?.error) {
        setError("Fel vid registrering med Google: " + result.error);
      } else if (result?.url) {
        router.push(result.url);
      } else {
        setError("Ingen redirect-URL returnerades från Google-registrering");
      }
    } catch (err: unknown) {
      if (err instanceof Error) {
        setError(
          "Ett oväntat fel inträffade vid Google-registrering: " + err.message
        );
      } else {
        setError("Ett oväntat fel inträffade vid Google-registrering.");
      }
    }
  };
